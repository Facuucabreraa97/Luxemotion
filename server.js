...
app.post('/api/talents/create', async (req, res) => {
    try {
        ...
        if (source_video_id) {
            ...
            // A video can't create a talent if it's already locked (meaning another talent from it was sold)
            if (sourceVideo.locked) {
                return res.status(403).json({
                    error: "SECURITY_BLOCK: The source asset is locked (already commercialized)."
                });
            }
            ...
            const { count: commercialCount, error: lineageError } = await supabaseAdmin
                ...
                .or('is_for_sale.eq.true,sales_count.gt.0'); // Corrected query for is_for_sale
