...
import {
    ...
    CreditCard, Search, MoreVertical, Check, X,
    Trash2, Ban, ShieldCheck, RefreshCw, LogOut
} from 'lucide-react';
        ...
        <div className="min-h-screen bg-black text-white font-sans flex text-sm">
            ...
            <aside className="w-64 border-r border-white/10 bg-[#0a0a0a] flex flex-col fixed h-full z-20">
                ...
                <div className="p-4 border-t border-white/10">
                    <button onClick={() => navigate('/app')} className="flex items-center gap-3 text-gray-400 hover:text-white transition w-full p-2 rounded-lg hover:bg-white/5">
            ...
            <main className="flex-1 ml-64 p-8">
                {loading && !stats ? (
                    <div className="text-center mt-20 text-gray-500 animate-pulse">Initializing Admin Core...</div>
                ) : (
                    <>
                        {/* Header */}
                        <header className="flex justify-between items-center mb-10">
                            ...
                            <button onClick={() => setRefreshTrigger(p => p + 1)} className="p-2 hover:bg-white/10 rounded-full transition">
                        ...
                        {activeTab === 'overview' && stats && (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                <StatCard label="Total Approved Users" value={stats.total_users} icon={<Users className="text-blue-500" />} />
                                <StatCard label="Pending Requests" value={stats.pending_requests} icon={<UserPlus className="text-yellow-500" />} />
                                <StatCard label="Credits vs Liabilities" value={stats.total_credits} icon={<CreditCard className="text-emerald-500" />} />
                                <StatCard label="Active (24h)" value={stats.active_users_24h} icon={<ShieldCheck className="text-purple-500" />} />
                            </div>
                        )}
                                            ...
                                            {filteredUsers.map(user => (
                                                <tr key={user.email} className="hover:bg-white/5 transition group">
                                                    <td className="p-4">
                                                        ...
                                                        <div className="text-xs text-gray-500">{user.user_id || 'Not Registered'}</div>
                                                    </td>
                                                    <td className="p-4">
                                                    ...
                                                    <td className="p-4 text-right flex justify-end gap-2 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                                                        ...
                                                        <ActionBtn
                                                            ...
                                                            onClick={() => handleAction(() => AdminService.toggleBan(user.email, user.whitelist_status !== 'rejected'), 'Ban Status toggled')}
                                                            color="text-orange-400 hover:bg-orange-400/10"
                                                        />
                                                        <ActionBtn
                                                            icon={<Trash2 size={14} />}
                                                            tooltip="Delete User"
                                                            onClick={() => handleAction(() => AdminService.deleteUser(user.email), 'User Deleted')}
                        ...
                        {activeTab === 'whitelist' && (
                            <div className="grid grid-cols-1 gap-4">
                                {pendingRequests.length === 0 ? (
                                    ...
                                    <div key={req.email} className="bg-[#111] p-6 rounded-xl border border-white/10 flex justify-between items-center">
                                        <div>
                                            <h3 className="font-bold text-lg">{req.email}</h3>
                                            <p className="text-gray-500 text-xs">Applied: {new Date(req.applied_at).toLocaleString()}</p>
                                        </div>
                                        <div className="flex gap-3">
                                            <button
                                                onClick={() => handleAction(() => AdminService.toggleBan(req.email, false), 'Approved!')}
                                            ...
                                            <button
                                                onClick={() => handleAction(() => AdminService.toggleBan(req.email, true), 'Rejected')}
...
const SidebarItem = ({ icon, label, active, onClick, count }: any) => (
    <button
        onClick={onClick}
        className={`w-full flex items-center justify-between p-3 rounded-lg transition-all ${active ? 'bg-white text-black font-bold shadow-lg shadow-white/10' : 'text-gray-400 hover:bg-white/5 hover:text-white'
            }`}
    >
        ...
        {count > 0 && (
            <span className="bg-emerald-500 text-black text-[10px] font-bold px-1.5 py-0.5 rounded-full">{count}</span>
        )}
    </button>
);
...
const StatusBadge = ({ status }: { status: string }) => {
    ...
    return (
        <span className={`px-2 py-1 rounded-full text-[10px] uppercase font-bold border ${colors[status as keyof typeof colors] || colors.pending}`}>
            {status}
        </span>
    );
};

const ActionBtn = ({ icon, onClick, color, tooltip }: any) => (
