...
export const EarningsDashboard = () => {
    ...
    const cards = [
        { title: 'Total Revenue', value: `${stats.totalError} CR`, icon: DollarSign, color: isVelvet ? 'text-[#C6A649]' : 'text-green-600' },
        { title: 'Assets Sold', value: stats.count, icon: Package, color: isVelvet ? 'text-white' : 'text-blue-600' },
        { title: 'Pipeline Value', value: `${stats.royalties.toFixed(0)} CR`, icon: TrendingUp, color: isVelvet ? 'text-purple-400' : 'text-purple-600', sub: 'Projected Royalties' }
    ];
    ...
    return (
        <div className={`p-6 lg:p-12 pb-32 animate-in fade-in slide-in-from-bottom-4`}>
            ...
            {/* Assets Grid */}
            <h3 className={`text-xl font-bold uppercase tracking-widest mb-8 ${isVelvet ? 'text-white' : 'text-gray-900'}`}>Sold Assets Portfolio</h3>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
                {earnings.map((item) => (
                    <div key={item.id} className={`group relative aspect-[9/16] rounded-[30px] overflow-hidden ${isVelvet ? 'bg-black/30' : 'bg-gray-100'} ${item.isExample ? 'border-2 border-dashed border-gray-500/30' : ''}`}>
                        ...
                        <div className="absolute top-4 left-4 right-4 flex justify-between items-start">
                            ...
                            {item.isExample && <div className="bg-blue-500/20 text-blue-300 px-2 py-1 rounded text-[8px] font-bold uppercase">Example</div>}
                        </div>
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center translate-y-4 opacity-0 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
                            ...
                            <span className={`text-[10px] font-bold uppercase tracking-[0.2em] mt-2 ${isVelvet ? 'text-white' : 'text-gray-900'}`}>Locked Asset</span>
                        </div>
                        ...
                        <div className="absolute bottom-0 inset-x-0 p-6 bg-gradient-to-t from-black/90 to-transparent">
                            ...
                            <div className="flex justify-between items-center text-[9px] font-bold uppercase text-white/60">
                                <span className="flex items-center gap-1"><BarChart3 size={10} /> Performance</span>
                                <span>High</span>
                            </div>
                        </div>
                    </div>
                ...
                {earnings.length === 0 && (
                    <div className="col-span-full py-20 text-center">
                        ...
                        <h3 className={`text-xl font-bold uppercase tracking-widest mb-2 ${isVelvet ? 'text-white' : 'text-gray-900'}`}>No Assets Sold Yet</h3>
                        <p className="text-xs text-gray-500 max-w-md mx-auto mb-8">Your portfolio is empty. Start casting models to generate revenue streams.</p>
