    ...
    <div className="min-h-screen bg-[#000] text-white flex flex-col font-sans selection:bg-white selection:text-black overflow-x-hidden">
      ...
      <header className="relative z-10 p-6 md:p-10 flex justify-center md:justify-start items-center max-w-[1800px] mx-auto w-full">
        <img
          ...
          alt="MivideoAI"
          className="h-10 md:h-12 object-contain opacity-100 transition-opacity"
        />
      </header>
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center px-4 pt-20 pb-32">
        ...
        <div className="mt-32 w-full max-w-[1600px] grid grid-cols-2 md:grid-cols-4 gap-4 px-4 opacity-40 hover:opacity-100 transition-opacity duration-700">
          {[
            {
              img: '/showcase/fashion.png',
              label: 'Fashion',
              prompt:
                'IG Story: A fashion influencer selfie, ring light reflection, trendy streetwear, natural skin texture, vertical 9:16.',
            },
            {
              img: '/showcase/tech.png',
              label: 'Tech',
              prompt:
                'YouTube Review: 4K frame of a tech reviewer holding a new smartphone, RGB studio background, depth of field.',
            },
            {
              img: '/showcase/lifestyle.png',
              label: 'Lifestyle',
              prompt:
                'Reel: Morning routine aesthetic, drinking matcha latte in a sunlit cafe, casual chic, shot on iPhone 15.',
            },
            {
              img: '/showcase/travel.png',
              label: 'Travel',
              prompt:
                'Vlog: Golden hour selfie at Santorini, wind in hair, GoPro wide angle style, authentic travel content.',
            },
          ].map((item, i) => (
            <div
              key={i}
              className={`aspect-[9/16] rounded-xl overflow-hidden relative group cursor-crosshair transform hover:scale-[1.02] transition-transform duration-500 ${i % 2 === 0 ? 'mt-0' : 'mt-12'}`}
            >
              <img
                src={item.img}
                alt={item.label}
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex flex-col justify-end p-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <span className="text-[10px] font-mono text-white/50 mb-2 uppercase tracking-widest">
                  PROMPT
                </span>
